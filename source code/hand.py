...
class Hand:
    ...
    def on_ball(self, balls): # return a list with all balls that collide with the hand hitbox
        return [ball for ball in balls if self.rect.colliderect(ball.rect)]


    def kill_balls(self, balls, score, sounds): # will kill the balls that collide with the hand when the left mouse button is pressed
        if self.left_click: # if left click
            for ball in self.on_ball(balls):
                ball_score = ball.kill(balls)
                score += ball_score
                sounds["slap"].play()
                if ball_score < 0:
