...
class Game:
    ...
    def spawn_balls(self):
        t = time.time()
        if t > self.balls_spawn_timer:
            self.balls_spawn_timer = t + REDS_SPAWN_TIME

            # increase the probability that the balls will be a green over time
            ...
            if random.randint(0, 100) < nb:
                self.balls.append(Green())
            else:
                self.balls.append(Red())

            # spawn a other red ball after the half of the game
    ...
    def update(self):
        ...
        if self.time_left > 0:
            ...
            self.score = self.hand.kill_balls(self.balls, self.score, self.sounds)
            for ball in self.balls:
                ball.move()
